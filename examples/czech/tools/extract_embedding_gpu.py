#!/usr/bin/env python3
"""GPU-accelerated speaker embedding extraction with progress bars and resume capability.

Extracts speaker embeddings using CAM++ model for CosyVoice fine-tuning.
Uses GPU (CUDA) for faster inference - approximately 5-20x faster than CPU.
Supports resuming from interrupted runs - already processed samples are skipped.

Environment: micromamba activate fish-speech

Usage:
    python extract_embedding_gpu.py --dir /path/to/data --onnx_path /path/to/campplus.onnx
"""
import argparse
import os
import sys
...
def main(args: argparse.Namespace) -> None:
    """Main extraction function with GPU acceleration and resume capability.
    ...
    print(f"Processing {len(utts_to_process):,} utterances ({skipped:,} already done)...", flush=True)

    # Setup ONNX session with GPU support
    option = onnxruntime.SessionOptions()
    option.graph_optimization_level = onnxruntime.GraphOptimizationLevel.ORT_ENABLE_ALL

    # Try GPU first, fall back to CPU
    available_providers = onnxruntime.get_available_providers()
    if "CUDAExecutionProvider" in available_providers and not args.cpu_only:
        providers = ["CUDAExecutionProvider", "CPUExecutionProvider"]
        print("Using GPU (CUDAExecutionProvider)", flush=True)
    else:
        providers = ["CPUExecutionProvider"]
        print("Using CPU (CUDAExecutionProvider not available or --cpu_only set)", flush=True)

    ort_session = onnxruntime.InferenceSession(args.onnx_path, sess_options=option, providers=providers)

    # Process sequentially (GPU inference is fast, no need for threading)
...
if __name__ == "__main__":
    print("=== GPU-ACCELERATED EMBEDDING EXTRACTION ===", flush=True)
    parser = argparse.ArgumentParser(description="Extract speaker embeddings with GPU acceleration and resume")
    ...
    parser.add_argument("--cpu_only", action="store_true", help="Force CPU even if GPU available")
